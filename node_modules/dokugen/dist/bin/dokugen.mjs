...
const backupReadme = async (readmePath) => {
    try {
...
const extractFullCode = async (projectFiles, projectDir) => {
    const fileGroups = {};
    ...
    const snippets = await Promise.all(Object.entries(fileGroups).map(async ([dir, files]) => {
        const dirSnippets = await Promise.all(files.map(async (file) => {
            try {
...
const loadGitignorePatterns = async (projectDir, ignoreDirs, ignoreFiles) => {
        ...
        try {
            ...
            for (const file of dirContents) {
                ...
                if (file.isDirectory()) {
                    ...
                    const shouldIgnore = [...ignoreFiles].some(pattern => matchesIgnorePattern(file.name, pattern));
                    if (!shouldIgnore) {
...
const generateReadme = async (projectType, projectFiles, projectDir, existingReadme, templateUrl) => {
    try {
        ...
        const getBackendDomain = await axios.get("https://dokugen-readme.vercel.app/api/get-server-url");
        ...
        return new Promise((resolve, reject) => {
            ...
            responseStream.on("error", async (err) => {
        ...
        console.error("\n Error Generating Readme", error.response?.data || error.message);
        const restoredContent = await restoreReadme();
